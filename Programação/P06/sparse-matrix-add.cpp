...
bool mesma_posicao(sm_entry element_a, sm_entry element_b) {
    if (element_a.row == element_b.row && element_a.col == element_b.col) return true;
    return false;
}

bool position_smaller_than(sm_entry element_a, sm_entry element_b) {
    if ((element_a.row < element_b.row)||( element_a.row == element_b.row && element_a.col < element_b.col)) return true;
    return false;
}
...
void sum(const smatrix& a, const smatrix& b, smatrix& r) {
    ...
    while (pos_a < a_length && pos_b < b_length) {
        if (mesma_posicao(a[pos_a], b[pos_b])) {
